...
the same loop discipline as `oc-app-architect`'s Generator/Evaluator (including its
3-round ceiling) — a first pass that fails two of the four axes is a fine and expected
outcome of an honest Evaluator.
...
The five archetypes are tabulated above; idempotency keys and late-data windows are
covered in the middleware/transform bullet above.
...
| **Wired** | the signal actually reaches its consumer and the read contract is stable. Freshness is *defined* here as `freshness_sla`; prod alerting on it is handed to `oc-monitoring-ops` — see Boundaries. |
...
records it. Max iterations: 3. **FAIL + max rounds** → escalate to the user with the
failing axis and every ground-truth reconciliation attempt; do not wire.
...
also hands the signal's `freshness_sla` (recorded in `signals/catalog.md`) to
`oc-monitoring-ops`: invoke `/oc-monitor alerts` with it so staleness gets a prod alarm.
This is a hand-off, not an intake monitoring-ops runs on its own — it has no
signal-forge reader, so the alert exists only once that command sets it up (Signal
Forge owns the *contract*; monitoring-ops owns the *alert*).
...
| **Alerting on signal staleness in prod** | `oc-monitoring-ops` — its SLO targets include a *Data freshness* row, and `/oc-monitor alerts` builds the alert. signal-forge **defines** `freshness_sla` and hands it over at `wire`; monitoring-ops has no automatic intake for it. |
...
> automatically. When the analytics doc warrants real instrumentation, run
> `/oc-signal frame` yourself after app-architect's spec gate; signal-forge then
> executes that plan.
...
| Wired | `signals[].consumer` resolved + freshness handoff to monitoring-ops (`/oc-monitor alerts`) |
...
2. **The Evaluator's job is to disprove the number.** A skeptical first pass that fails
   axes is fine; a confirmation-biased pass on all four is a failure of the gate.
