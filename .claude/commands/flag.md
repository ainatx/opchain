...
| `skills.coverage.<id>.enabled` | Reserved per-pack oc-stack-forge coverage flag; nothing reads it yet, so flipping it has no effect (helper, generated from `skills/oc-stack-forge/packs/<id>/pack.yml`) |
