...
- **Every deploy ends with a baseline check.** `scripts/deploy.mjs` compares the live version with `.github/monitoring/release-baseline.json` and prints a stale-baseline warning when they differ. That warning is the moment the Canary and Deploy lag monitors start failing; follow `docs/runbooks/cloudflare-challenge.md` → "After an intentional deployment" steps 4–6 (record the new deployment/version ids, verify locally with credentials, open the baseline PR). On 2026-09-05 production moved off the baseline without that refresh and the scheduled monitors were red for a week. Deploy lag now files its tracking issue on a mismatch instead of dying before the issue step.
...
`.github/workflows/lighthouse.yml` runs Lighthouse/Axe budgets on PR builds (not against deployed environments). It serves `site/dist` through `scripts/lhci-preview.mjs`, not `astro preview`: the header's live-version chip fetches `/api/health` on every page, and a preview with no Worker routes logged a console 404 that cost the Best Practices score one point on every route. The preview stubs that one route and 404s every other `/api/*` path, so an unstubbed client-side fetch still shows up as the console error it would be. Every route in `lighthouserc.cjs` gates at `error` level.
