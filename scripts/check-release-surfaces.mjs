...
export function checkReleaseSurfaces() {
  ...
  if (releaseVersion) {
    if (existsSync(p("release-preview.json"))) {
      const preview = JSON.parse(readFileSync(p("release-preview.json"), "utf8"));
      if (preview.schemaVersion !== 1 || preview.status !== "staging-preview" || preview.releaseDate !== null || preview.baseline !== releaseVersion) errors.push("Invalid staging preview release metadata");
